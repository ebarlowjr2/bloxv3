...
export default function IdePage() {
  ...
  const [status, setStatus] = useState<'unknown' | 'reachable' | 'unreachable'>('unknown');
  ...
  useEffect(() => {
    ...
    fetch(ideUrl, { mode: 'no-cors', signal: controller.signal })
      .then(() => {
        if (isMounted) setStatus('reachable');
      })
      .catch(() => {
        if (isMounted) setStatus('unreachable');
      })
